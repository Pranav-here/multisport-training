...
export const fetchDashboardClips = cache(async (userId?: string): Promise<DashboardClip[]> => {
  ...
  try {
    ...
    const clipRows = (clips ?? []) as TrainingClipRow[];
    ...
    const clipsWithMetrics: DashboardClip[] = await Promise.all(
      clipRows.map(async (clip) => {
        ...
        return {
          ...
          user: {
            displayName: (clip.user as UserProfileData | null)?.display_name ?? null,
...
export const fetchLeaderboard = cache(async (): Promise<LeaderboardEntry[]> => {
  ...
  try {
    ...
    const leaderboardRows = (leaderboard ?? []) as TrainingLeaderboardRow[];
    ...
    const mapped: LeaderboardEntry[] = leaderboardRows.map((entry, index) => {
      ...
      return {
        ...
        userName: userData?.display_name ?? userData?.username ?? "Athlete",
        userAvatar: userData?.avatar_url ?? "/placeholder.svg",
        score: entry.score ?? 0,
        school: userData?.username ? `@${userData.username}` : "",
        sport: sportData?.name ?? "Multi-Sport",
      };
    });
